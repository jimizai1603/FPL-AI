def compute_xp_and_price_risk(row):
                    ...
                    # 1. Expected Points Proxy Math: Form (50%) + xGI (50%) adjusted by FDR
                    ...
                    # 2. Dynamic Price Risk Evaluation
                    net_trans = row["Net_Transfers"]
                    ...
                    # 3. Mini-League Ownership context
                ...
                # Squad Breakdown Display
                df_squad_display = df_squad[
                    ["web_name", "element_type", "Cost_m", "Form_Float", "Est_xP", "Next_FDR", "Price_Risk", "ML_EO%"]
                ].copy()
                df_squad_display["Pos"] = df_squad_display["element_type"].map(POSITION_MAP)
                df_squad_display = df_squad_display.rename(columns={
                    "web_name": "Player",
                    "Cost_m": "Cost (£m)",
                    "Form_Float": "Form",
                    "Est_xP": "Est. xP (GW" + str(next_gw) + ")",
                    "ML_EO%": "League Ownership %"
                })

                st.markdown(f"#### 📋 Current Squad Analytics (GW {next_gw} Outlook)")
                st.dataframe(
                    df_squad_display[["Player", "Pos", "Cost (£m)", "Form", "Est. xP (GW" + str(next_gw) + ")", "Next_FDR", "Price_Risk", "League Ownership %"]],
                    use_container_width=True,
                    hide_index=True
                )

                # Executive Algorithmic Recommendations
                st.markdown("---")
                ...
                sell_candidates = df_squad[
                    (df_squad["Est_xP"] < 3.0) | (df_squad["Price_Risk"].str.contains("Drop|Sell"))
                ].sort_values(by="Est_xP", ascending=True)

                with c_rec1:
                    st.markdown("#### 🚨 Priority Exit Targets (Sell Pressure / Low xP)")
                ...
                # Transfer In Candidates (High Market Momentum + Form)
                ...
                # Exclude existing squad members
                ...
                ].sort_values(by=["transfers_in_event", "Form_Float"], ascending=[False, False])
                ...
                st.error("Unable to load team composition. Please confirm your FPL Team ID.")
    else:
        st.info("Please input your FPL Team ID above to execute squad analysis.")
